...
import { supprimerProduitAction } from "../../redux/actions/produitActions";
import { useDispatch } from "react-redux";

export default function Produit({ produit, categorie }) {
  const dispatch = useDispatch();

  function handleDelete() {
    const confirmation = confirm("Are U sure");
    confirmation && dispatch(supprimerProduitAction(produit.id));
  }

  return (
    <div className="relative w-96 h-auto border border-gray-200 shadow-md p-8 rounded-xl bg-white flex flex-col justify-between mx-auto transition-transform transform hover:scale-105 hover:shadow-2xl duration-300 ease-in-out cursor-pointer">
      ...
      <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center space-x-4 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-xl">
        ...
        <Link
          to={`/produits/modifier/${produit.id}`}
